...
function requestMetricUpdate() {
  ...
  if (dataRange){
    ...
    var METRIC_DATA_COLUMNS = 10; // number of data columns that there should be
...
function updateMetrics(metrics) {
  ...
  for (var d = 0; d < metrics.length; d++) {
    ...
    if (metrics[d][0]) {
      ...
      var account = metrics[d][1];
      var property = metrics[d][2];
      var name = metrics[d][3], index = metrics[d][4], scope=metrics[d][5], type=metrics[d][6], min=metrics[d][7], max=metrics[d][8], active=metrics[d][9];
