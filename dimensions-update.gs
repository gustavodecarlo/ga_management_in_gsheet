...
function requestDimensionUpdate() {
  ...
  if (dataRange) {
    ...
    var DIMENSION_DATA_COLUMNS = 7; // number of data columns that there should be
    ...
    if (dataRows > 0 && dataColumns == DIMENSION_DATA_COLUMNS) {
      ...
      Logger.log(updateDimensionResponse);
...
function updateDimensions(dimensions) {
  ...
  for (var d = 0; d < dimensions.length; d++) {
    ...
    if (dimensions[d][0]) {
      var account = dimensions[d][1];
      var property = dimensions[d][2];

      var name = dimensions[d][3], index = dimensions[d][4], scope=dimensions[d][5], active=dimensions[d][6];
